...
homeaway <- function(df){
    ...
    } else
    {
     ...
     rbind(df %>% dplyr::select(Date,Season,team=home,opp=visitor,gf=hgoal,ga=vgoal,teamconf=hconf,oppconf=vconf,round,leg, gfaet=hgoalaet, gaaet=vgoalaet, penfaet=hpen, penaaet=vpen) %>% dplyr::mutate(venue="home"),
           df %>% dplyr::select(Date,Season,team=visitor,opp=home,gf=vgoal,ga=hgoal,teamconf=vconf,oppconf=hconf,round,leg, gfaet=vgoalaet, gaaet=hgoalaet, penfaet=vpen, penaaet=hpen) %>% dplyr::mutate(venue="away")
     ) %>% dplyr::arrange(team,Date)
    }
