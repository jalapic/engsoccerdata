...
#' holland_current()
#' @export
holland_current <- function(Season = 2025){
  ...
  # Standardize
  df1 <- rbind(engsoccerdata::getCurrentData(n1, 1, 1, Season = myseason))

  # Date class for checks/keys
  ...
  # True overlap warning (holland has no division col)
  hol <- engsoccerdata::holland
  hol_key <- paste(hol$Date, hol$Season, hol$tier, hol$home, hol$visitor, sep = "|")
  df1_key <- paste(as.character(df1$Date), df1$Season, df1$tier, df1$home, df1$visitor, sep = "|")

  n_overlap <- sum(df1_key %in% hol_key)
  if (n_overlap > 0) {
    warning(sprintf(
      "The returned dataframe contains %d match(es) already included in the 'holland' dataset.",
      n_overlap
    ))
  }

  # Optional: align columns to stored dataset schema
  target_cols <- colnames(engsoccerdata::holland)
  df1 <- df1[target_cols]
